        ...
        SubTxAwareParticipant
{
    ...
    public CompositeTransaction suspend () throws SysException
    {
        ...
        if ( ret != null ) {
            ...
            suspendInTransactionService(ret);
        } else {
        	if(LOGGER.isDebugEnabled()){
    ...
    private void suspendInTransactionService(CompositeTransaction ret) {
        try {
            getTransactionService().transactionSuspended(ret);
        } catch (Exception e) {
            //happens if already shutdown
            //ignore: courtesy method - but log
            LOGGER.logWarning("Unexpected error on suspend - ignoring...", e);
        }
    }
    
    private void resumeInTransactionService(CompositeTransaction ret) {
        try {
            getTransactionService().transactionResumed(ret);
        } catch (Exception e) {
            //happens if already shutdown
            //ignore: courtesy method - but log
            LOGGER.logWarning("Unexpected error on resume - ignoring...", e);
        }
    }
